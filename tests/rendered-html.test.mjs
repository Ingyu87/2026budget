...
test("builds a standard Next.js deployment and keeps the dashboard content", async () => {
  ...
  const [page, caseData, caseCatalog, edtechData, styles, layout, packageJson] = await Promise.all([
  ...
  assert.match(edtechData, /"all": 54/);
  assert.doesNotMatch(edtechData, /초등학교|중학교|고등학교|"school"/);
